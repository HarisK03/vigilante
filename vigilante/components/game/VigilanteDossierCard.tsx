...
const [selected, setSelected] = useState(vigilantes[0]);
...
function StatBar({
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="h-2 rounded-full bg-black/40 ring-1 ring-amber-900/35 overflow-hidden">
        <div
  ...
  return (
    <AnimatePresence>
      {open && character ? (
        <>
          <motion.div
            className="absolute inset-0 z-[110] bg-black/45"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={onClose}
          ...
          <motion.aside
            ...
            className="absolute left-6 top-6 bottom-6 z-[120] w-[min(34vw,460px)] min-w-[340px] overflow-hidden rounded-2xl border border-amber-900/40 bg-black/70 text-amber-100 shadow-[0_18px_70px_rgba(0,0,0,0.55)] backdrop-blur-md"
          >
            <div
            ...
            <div className="relative flex h-full flex-col">
                  ...
                  <div className="rounded-xl border border-amber-900/30 bg-black/25 p-4">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {(character.traits ?? []).length > 0 ? (
                        ...
                        <span className="text-sm text-amber-200/45">No listed traits.</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="rounded-xl border border-amber-900/30 bg-black/25 p-4">
                    ...
                    <ul className="mt-3 space-y-2 text-sm text-amber-100/75">
                      {(character.equipment ?? []).length > 0 ? (
                        ...
                        <li className="text-amber-200/45">No recorded equipment.</li>
                      )}
                    </ul>
                  </div>
                </div>
              ...
              <div className="border-t border-amber-900/30 px-5 py-4">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button
                    type="button"
                    onClick={() => onDispatch?.(character.id)}
