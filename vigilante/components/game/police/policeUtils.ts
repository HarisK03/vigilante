...
/**
 * Resample the path so segment lengths are more uniform.
 * This makes movement look smoother and reduces the "speed burst" effect.
 */
export function resamplePath(path: LatLngTuple[], targetStepMeters = 28) {
	...
	for (let i = 0; i < clean.length - 1; i += 1) {
		...
		if (segmentMeters <= targetStepMeters) {
			out.push(b);
			continue;
		}

		const steps = Math.ceil(segmentMeters / targetStepMeters);
...
export function getSegmentDurationMs(
	...
	// Response is fastest, rejoin is medium, patrol is slowest.
	const speedMps =
		mode === "responding" ? 17 : mode === "rejoining" ? 11 : 7.5;

	const base = Math.round((meters / speedMps) * 1000);

	if (mode === "responding") {
		return Math.max(650, Math.min(4500, base));
	}

	if (mode === "rejoining") {
		return Math.max(700, Math.min(5500, base));
	}
	...
	return Math.max(800, Math.min(7000, base));
}
