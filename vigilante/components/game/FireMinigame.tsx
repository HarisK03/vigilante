...
function makeFire(nearY?: number, health = 1.0): FireBall {
  const y =
    nearY !== undefined
      ? Math.max(0, Math.min(BAR_H - FIRE_PX, nearY + (Math.random() - 0.5) * 80))
      : Math.random() * (BAR_H - FIRE_PX);
  const speed = BASE_SPEED * (0.7 + Math.random() * 0.7);
  return { y, vy: Math.random() < 0.5 ? speed : -speed, health, born: performance.now(), deadAt: null };
}
...
function drawRoundRect(
  ...
  if (typeof (ctx as any).roundRect === "function") {
    ...
    ctx.moveTo(x + c, y);
    ctx.lineTo(x + w - c, y);     ctx.arcTo(x + w, y,     x + w, y + h, c);
    ctx.lineTo(x + w, y + h - c); ctx.arcTo(x + w, y + h, x,     y + h, c);
    ctx.lineTo(x + c, y + h);     ctx.arcTo(x,     y + h, x,     y,     c);
    ctx.lineTo(x, y + c);         ctx.arcTo(x,     y,     x + w, y,     c);
    ctx.closePath();
  }
}
...
export default function FireMinigame({ onSuccess, onFailure }: Props) {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      s.rectY  += s.rectVY;
      if (s.rectY <= 0)              { s.rectY = 0;               s.rectVY =  Math.abs(s.rectVY) * BOUNCE; }
      if (s.rectY + RECT_H >= BAR_H) { s.rectY = BAR_H - RECT_H; s.rectVY = -Math.abs(s.rectVY) * BOUNCE; }

      // Fire movement
      for (const f of s.fires) {
        f.y += f.vy;
        if (f.y <= 0)               { f.y = 0;               f.vy =  Math.abs(f.vy) * (0.65 + Math.random() * 0.5); }
        if (f.y + FIRE_PX >= BAR_H) { f.y = BAR_H - FIRE_PX; f.vy = -Math.abs(f.vy) * (0.65 + Math.random() * 0.5); }
      ...
      if (now - s.lastSpread > SPREAD_MS && livingFires.length < MAX_FIRES) {
        const parent = livingFires[Math.floor(Math.random() * livingFires.length)];
      ...
      s.fires = s.fires.filter(f => f.deadAt === null || now - f.deadAt < FADE_OUT_MS);
      ...
      if (livingCount > 0) {
        s.progress += (GAIN_PER_FIRE * covered - DRAIN_PER_FIRE * uncovered) * dt;
        s.progress  = Math.max(0, Math.min(1, s.progress));
      }
      setUiProgress(s.progress);

      if (s.progress >= 1) { s.done = true; setResult("win");  return; }
      if (s.progress <= 0) { s.done = true; setResult("lose"); return; }
      ...
      if (ectx) {
        for (const f of s.fires) {
          ...
          const fadeOut = f.deadAt !== null ? 1 - (now - f.deadAt) / 400 : 1;
  ...
  const meterColor =
    uiProgress > 0.6 ? "#d97706" :
    uiProgress > 0.3 ? "#b45309" : "#b91c1c";
      ...
      >
        ...
        >
          ...
          <div
            ...
            style={{ borderBottom: "1px solid rgba(120,53,15,0.40)" }}
          >
            <div
          ...
          {/* ── READY SCREEN ── */}
          {phase === "ready" && (
            <div className="flex flex-col gap-5 px-4 py-4" style={{ width: 320 }}>
              ...
              <div className="flex gap-4 items-center justify-center">
                <div className="flex flex-col items-center gap-1">
                  ...
                  <div style={{
                    ...
                    <div style={{ height: "50%", background: "#b45309", borderRadius: 99 }} />
                  </div>
                </div>
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    <span style={{ fontSize: 6, color: "#bfefff", opacity: 0.65, fontFamily: "monospace" }}>WATER</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2.5">
                {[
                  { label: "HOLD",    col: "rgba(217,119,6,0.90)",   text: "Click and hold to push the water bar upward." },
                  { label: "RELEASE", col: "rgba(96,165,250,0.90)",  text: "Let go to drop it. It bounces off the edges." },
                  { label: "COVER",   col: "rgba(74,222,128,0.90)",  text: "Keep fires inside the bar to extinguish them. Meter fills while covered, drains while exposed." },
                  { label: "SPREAD",  col: "rgba(248,113,113,0.90)", text: "Fires split and multiply over time. Don't let the meter empty." },
                ].map(({ label, col, text }) => (
              ...
              <div className="h-[3px] w-full rounded-full overflow-hidden" style={{ background: "rgba(120,53,15,0.40)" }}>
                <div className="h-full rounded-full" style={{ width: "70%", background: "rgba(217,119,6,0.70)" }} />
              </div>
              ...
              <button
                ...
                onMouseEnter={e => {
                  ...
                  b.style.borderColor = "rgba(220,38,38,0.80)";
                  b.style.color = "rgba(254,202,202,1)";
                  b.style.background = "rgba(153,27,27,0.55)";
                }}
                onMouseLeave={e => {
                  const b = e.currentTarget as HTMLButtonElement;
                  b.style.borderColor = "rgba(153,27,27,0.65)";
          ...
          {result && (
            <div
              ...
              style={{ borderTop: "1px solid rgba(120,53,15,0.40)" }}
            >
              <div
                className="h-5 w-5 rounded-full border flex items-center justify-center text-[11px] font-bold flex-shrink-0"
                style={{
                  borderColor: result === "win" ? "rgba(21,128,61,0.7)"  : "rgba(153,27,27,0.7)",
                  background:  result === "win" ? "rgba(21,128,61,0.25)" : "rgba(153,27,27,0.25)",
                  color:       result === "win" ? "rgba(134,239,172,0.9)": "rgba(252,165,165,0.9)",
                }}
              >
                {result === "win" ? "✓" : "✕"}
              </div>
              ...
              >
                {result === "win" ? "Fire Contained" : "Out of Control"}
              </span>
            </div>
          )}
