...
import type { VigilanteMode, VigilanteUnit } from "./vigilanteTypes";
...
export function getSpeedMps(unit: VigilanteUnit, mode: VigilanteMode) {
  switch (mode) {
    case "responding":
...
export function getUnitPosition(unit: VigilanteUnit, now: number): LatLngTuple {
...
export function advanceUnit(unit: VigilanteUnit, now: number) {
...
export function restartLoop(unit: VigilanteUnit, now: number) {
  if (unit.path.length < 2) return unit;

  const nextUnit: VigilanteUnit = {
    ...unit,
    responseOverrideMps:
...
export function getRemainingVisiblePath(
  unit: VigilanteUnit,
  now: number,
) {
...
export function getRemainingEtaMs(unit: VigilanteUnit, now: number) {
