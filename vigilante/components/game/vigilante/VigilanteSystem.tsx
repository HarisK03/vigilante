...
type Props = {
  ...
  onVigilanteEtaUpdate?: (items: Array<{
    unitId: string;
    name: string;
    incidentId: string;
    etaMs: number;
...
const DEFAULT_RESPONSE_MPS = 14.0;
...
function dedupeAdjacentVigilantePath(path: LatLngTuple[]) {
  ...
  for (let i = 1; i < path.length; i += 1) {
    ...
    if (
      distanceMeters(last, point) >= VIGILANTE_ROUTE_MIN_POINT_SPACING_METERS
    ) {
      result.push(point);
      continue;
    }
  ...
  while (changed) {
    ...
    outer: for (let i = 0; i < next.length - 3; i += 1) {
      ...
      for (
        ...
        if (
          revisitDistance >
          VIGILANTE_ROUTE_REVISIT_PROXIMITY_METERS
        ) {
          continue;
        }
...
function hasEarlyReturnHook(
  ...
  for (let i = 1; i < lookaheadEnd; i += 1) {
    ...
    if (
      detourToEnd <
      startToEndDistance + VIGILANTE_ROUTE_HOOK_AWAY_FROM_TARGET_MARGIN_METERS
    ) {
      continue;
    }
...
// Helper to calculate adjusted travel time based on vigilante speed stat
function calculateTravelMs(distanceMeters: number, patrolMps: number): number {
  return (distanceMeters / Math.max(patrolMps, 0.1)) * 1000;
}
...
export default function VigilanteSystem({
  ...
  const buildVigilanteConfig = (sheet: typeof ownedVigilanteSheets[0], index: number) => {
  ...
  useEffect(() => {
    ...
    if (prevIds.length > 0 && currentIds.length === prevIds.length && currentIds.every((id, i) => id === prevIds[i])) {
      return;
    }
    ...
    const initialUnits: VigilanteUnit[] = ownedVigilanteSheets.map((sheet, index) => {
      const config = buildVigilanteConfig(sheet, index);
      const patrolRouteId = config.patrolRouteId;
      const initialLoop = ensureClosedLoop(VIGILANTE_PATROL_ANCHORS[patrolRouteId] ?? VIGILANTE_PATROL_ANCHORS.default);
      ...
      void (async () => {
        try {
          const def = {
            ...
            anchors: VIGILANTE_PATROL_ANCHORS[routeId] ?? VIGILANTE_PATROL_ANCHORS.default,
          };
          ...
          console.warn("Failed to build vigilante patrol loop:", error);
        }
      })();
    }
  }, [ownedVigilanteSheets]);
  ...
  function startResponseJob(unit: VigilanteUnit, incident: VigilanteIncident) {
        ...
        setUnits((prev) => {
          ...
          const tunedResponseMps =
            desiredResponseMps > 0
              ? Math.max(baseResponseMps * 0.75, Math.min(baseResponseMps * 4, desiredResponseMps))
              : baseResponseMps;
  ...
  function startRejoinJob(unit: VigilanteUnit) {
    ...
    void (async () => {
      try {
        ...
        const nearestIndex = findNearestPointIndex(safeLoop, currentPos);
  ...
  useEffect(() => {
    ...
    const activeIncidents = incidents.filter(
      (incident) => incident.status === "active" && now < incident.expiresAt,
    );
    if (activeIncidents.length === 0) return;

    // Find incidents that have deployed vigilantes but not yet assigned
    const incidentsWithDeployedVigilantes = activeIncidents.filter((incident) => {
