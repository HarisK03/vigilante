...
def user_can_access_specific_endpoints_as_a_verified_volunteer(tier2_login):
    # update with endpoints
    valid_endpoints = ["/api/report"]

    for endpoint in valid_endpoints:
        response = requests.get(BASE_URL + endpoint, cookies=tier2_login.cookies, timeout=10)
